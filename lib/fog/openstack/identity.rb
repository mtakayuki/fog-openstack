    ...
    class OpenStack < Fog::Service
      ...
      def self.new(args = {})
        if self.inspect == 'Fog::Identity::OpenStack'
          identity = super
          service = identity.v3? ? Fog::Identity::OpenStack::V3.new(identity.config) : Fog::Identity::OpenStack::V2.new(identity.config)
        else
      ...
      class Real
        DEFAULT_SERVICE_TYPE_V3 = %w(identity_v3 identityv3 identity).map(&:freeze).freeze
        DEFAULT_SERVICE_TYPE    = %w(identity).map(&:freeze).freeze
        ...
        def initialize_endpoint_path_matches(options)
          @openstack_endpoint_path_matches = options[:openstack_endpoint_path_matches] if options[:openstack_endpoint_path_matches]
        end
